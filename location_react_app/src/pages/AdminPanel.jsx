...
const AdminPanel = () => {
    ...
    async function fetchAnnonces() {
        ...
        setListeAnnonces(await fetch('http://localhost:8080/signalements', requestOptions)
            ...
            <div className="" style={{backgroundColor: "#D9D9D9", height: "100%", width: "1200px", padding: "40px"}}>
                ...
                setdisplayNouvelleAnnonce(true);
                        ...
                        <div style={{margin: "40px"}}>
                            <table className="table table-striped boxshadowing2">
                                ...
                                {listeAnnonces?.map((a, index) => (
                                    <tr key={index}>
                                        <td>{a.annonce.titre}</td>
                                        <td>{a.raison}</td>
                                        <td>{a.annonce.status == 1 ? ("Consulté") : ("À consulter")}</td>
                                        <td>{new Date(a.dateSignalement).toLocaleDateString()}</td>
                                        <td>
                                            <div style={{
                                                display: "flex",
                                                ...
                                                <label className="switch">
                                                    <input type="checkbox"
                                                           defaultChecked={a.annonce.status === 1} onChange={(e) => {
                                                        console.log(e.target.checked);
                                                        if (e.target.checked) {
                                                            changerAnnonceStatus(e, a.annonce.id, 0)
                                                        } else {
                                                            changerAnnonceStatus(e, a.annonce.id, 1)
                                                        }
                                                    }}/>
                    ...
                    : displayModifierAnnonce ?
                        <>
                            <ModificationAnnonce idAnnonce={annonceModifier}
                                                 setDisplayModifierAnnonce={setDisplayModifierAnnonce}/>
                        </>
                        :
                        <>
