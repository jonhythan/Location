...
const NouveauCompte = () => {
    ...
    return (
        <div>
            ...
            <h4 className="text-center text-secondary">Afin d'enregistrer votre compte, veuillez entrer des informations
                valides</h4>
            <form onSubmit={handleSubmit} className="container container-login">
                <div className="row px-5 p-3">
                    <input type="text" value={prenom} onChange={e => setPrenom(e.target.value)} placeholder="Prénom"
                           required className='col '/>
                    <input type="text" value={nom} onChange={e => setNom(e.target.value)} placeholder="Nom" required
                           className='col'/>
                </div>
                <div className="row px-5 p-3">
                    <input type="text" value={telephone} onChange={e => setTelephone(e.target.value)}
                           placeholder="Téléphone" pattern="[1-9]{10}" required className='col'/>
                    <span className='col'></span>
                </div>
                <div className="row px-5 p-3">
                    <input type="email" value={courriel} onChange={e => setCourriel(e.target.value)} placeholder="Émail"
                           required className='col'/>
                    <span className='col'></span>
                </div>
                <div className="row px-5 p-3">
                    <input type="password" value={mdp} onChange={e => setMdp(e.target.value)} placeholder="Mot de passe"
                           required className='col'/>
                    <input type="password" value={cmdp} onChange={e => setCmdp(e.target.value)}
                           placeholder="Confirmation mot de passe" required className='col'/>
                </div>
                ...
                <div className="row px-5 p-3">
                    <input type="text" value={numRue} onChange={e => setNumRue(e.target.value)} pattern="[1-9]{}"
                           placeholder="No" className='col-1' required/>
                ...
                <div className="row px-5 p-3">
                    <input type="text" value={province} onChange={e => setProvince(e.target.value)}
                           placeholder="Province" className='col-4' required/>

                    <span className='col-3'><input type="text" value={codePostal}
                                                   onChange={e => setCodePostal(e.target.value.toLocaleUpperCase())}
                                                   placeholder="Code Postal"
                                                   pattern="[a-zA-Z]{1}[1-9]{1}[a-zA-Z]{1}-[1-9]{1}[a-zA-Z]{1}[1-9]{1}"
                                                   required/>&nbsp;A1A-A1A</span>
